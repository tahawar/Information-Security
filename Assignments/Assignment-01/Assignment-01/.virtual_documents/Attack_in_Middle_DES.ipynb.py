# get_ipython().getoutput("pip install pycryptodome")
# get_ipython().getoutput("pip install pycryptodome-test-vectors")
# get_ipython().getoutput("python -m Crypto.SelfTest")
# get_ipython().getoutput("pip install pycryptodomex")
# get_ipython().getoutput("pip install pycryptodome-test-vectors")
...
def generateTable(plaintext, effKeyLength):
    ...
    for p in perms:
        ...
        p=pad+p
        if(len(p) get_ipython().getoutput("= 16):")
...
main()
    ...
    d = k.encrypt(data)
    print "Encrypted: get_ipython().run_line_magic("r"", " % d")
    print "Decrypted: get_ipython().run_line_magic("r"", " % k.decrypt(d)")
    ...
    def __init__(self, mode=ECB, IV=None, pad=None, padmode=PAD_NORMAL):
        ...
        if IV and len(IV) get_ipython().getoutput("= self.block_size:")
    ...
    def setIV(self, IV):
        ...
        if not IV or len(IV) get_ipython().getoutput("= self.block_size:")
            raise ValueError("Invalid Initial Value (IV), must be a multiple of " + str(self.block_size) + " bytes")
        IV = self._guardAgainstUnicode(IV)
        self._iv = IV
    ...
    def __init__(self, key, mode=ECB, IV=None, pad=None, padmode=PAD_NORMAL):
        # Sanity checking of arguments.
        if len(key) get_ipython().getoutput("= 8:")
    ...
    def __String_to_BitList(self, data):
        ...
        for ch in data:
            i = 7
            while i >= 0:
                if ch & (1 << i) get_ipython().getoutput("= 0:")
                    result[pos] = 1
                else:
                    result[pos] = 0
                pos += 1
    ...
    def crypt(self, data, crypt_type):
        ...
        if not data:
            return ''
        if len(data) % self.block_size get_ipython().getoutput("= 0:")
            ...
            # print "Len of data: get_ipython().run_line_magic("f"", " % (len(data) / self.block_size)")
        ...
        while i < len(data):
            ...
            #lines += 1
                ...
                #print "Cached result for: get_ipython().run_line_magic("s"", " % data[i:i+8]")
            #	cached += 1
        ...
        # print "Lines: get_ipython().run_line_magic("d,", " cached: %d\" % (lines, cached)")
    ...
    def setKey(self, key):
        ...
        if len(key) get_ipython().getoutput("= self.key_size:")
        ...
        if self.getMode() == CBC:
            ...
            if len(self.getIV()) get_ipython().getoutput("= self.block_size:")
...
getsizeof("MynameMuhammadif")


len("W3110D0N30GUY500")


len("MynameMuhammadifMynameMuhammadif")
